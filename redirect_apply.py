...
def gate_state(corpus_path: Path | None = None) -> dict[str, Any]:
    ...
    disagreements_needed = max(
        redirect_shadow.DISAGREEMENT_OUTCOME_TARGET - disagreements, 0
    )
    return {
...
def link_applied_outcomes(
    *, dry_run: bool = False, corpus_path: Path | None = None, conn=None
    ...
    pending = [
        row for row in pending_outcome_links(conn=conn)
    ...
    for row in pending:
        ...
        if synced:
            _heartbeat("outcome", ref=str(row["influenced_run_id"]),
                       metadata={"role_run_id": row["role_run_id"]})
    return {"pending": len(pending), "linked": len([r for r in linked if r.get("linked")]),
            "dry_run": bool(dry_run), "links": linked[:20]}
...
def authorize(
    *,
    ...
    delegate_steps = [
        step for step in plan_obj.get("steps") or []
...
def _applied_history(corpus_path: Path, *, now: int | None = None) -> tuple[set[str], int]:
    ...
    today = len([
        e for e in events
        if e.get("applied") and int(e.get("ts") or 0) >= stamp - DAY_SECONDS
    ])
...
def apply_one(
    *,
    ...
    _heartbeat("match", ref=str(report.get("target") or ""),
               metadata={"bootstrap_needed": gate["bootstrap_needed"]})

    result = runner(report, acceptance_criteria, backend=backend, dispatch=True,
                    lane=report.get("lane"), task_type=report.get("task_type"))
    ...
    redirect_shadow.record_proposal(
        role_run_id=role_run_id, report=report, acceptance_criteria=acceptance_criteria,
        proposal=result.get("proposal"), baseline=result.get("baseline") or {},
        decision_source=result.get("decision_source") or "", errors=result.get("errors"),
        backend=result.get("backend"), backend_run_id=result.get("backend_run_id"),
        plan=plan_obj, corpus_path=corpus, source="live-dispatch",
    )
    ...
    if authorization["would_mutate"]:
        _heartbeat("invocation", ref=authorization["target"],
                   metadata={"role_run_id": role_run_id})
        ...
        if apply_result.get("applied"):
            _heartbeat("success", ref=authorization["target"],
                       metadata={"role_run_id": role_run_id})
    redirect_shadow.record_apply(
        role_run_id=role_run_id, target=authorization["target"],
        plan_action=plan_obj.get("action"), authorization=authorization,
        apply_result=apply_result, dry_run=not authorization["would_mutate"],
        corpus_path=corpus,
    )
    return {"gate": gate, "authorization": authorization, "apply_result": apply_result,
            "role_run_id": role_run_id, "target": authorization["target"],
            "decision_source": result.get("decision_source"), "errors": result.get("errors") or []}
...
def screen_candidates(
    *, report_dir: Path | None = None, corpus_path: Path | None = None, pid_checker=None
    ...
    rows = []
    for path in sorted(Path(directory).glob("*.keepalive-supervisor-report.json")) \
            if Path(directory).is_dir() else []:
        try:
            report = json.loads(path.read_text(encoding="utf-8"))
        except Exception:                                          # noqa: BLE001
            continue
        rows.append(screen_report(report, gate=gate, applied_targets=applied_targets,
                                  applies_today=applies_today, pid_checker=pid_checker))
    return {"reports_seen": len(rows), "gate": gate,
            "passing_screen": len([r for r in rows if r["passes_screen"]]),
            "report_dir": str(directory), "candidates": rows,
...
def apply_candidates(
    *,
    ...
    if not flag_enabled(env) and not (dry_run and spend_offloads):
        return {"reports_seen": 0, "considered": 0, "authorized": 0, "applied": 0,
    ...
    directory = report_dir or keepalive_supervisor.DEFAULT_STAGE2_REPORT_DIR
    reports = sorted(Path(directory).glob("*.keepalive-supervisor-report.json")) \
        if Path(directory).is_dir() else []
    results: list[dict] = []
    ...
    for path in reports:
        ...
        try:
            ...
            results.append({"target": path.name, "authorized": False, "applied": False,
        ...
        outcome = apply_one(
            ...
            backend=backend, corpus_path=corpus_path,
            env={} if dry_run else env,
            role_runner=role_runner, apply_runner=apply_runner, pid_checker=pid_checker,
        )
        ...
        results.append({"target": outcome["target"], "authorized": auth["allowed"],
                        "applied": was_applied, "blocks": auth["blocks"],
                        "role_run_id": outcome["role_run_id"]})
    return {"reports_seen": len(reports), "considered": considered, "authorized": authorized,
            "applied": applied, "report_dir": str(directory), "results": results}
...
def status(corpus_path: Path | None = None, *, env: dict | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "unlinked_applied_outcomes": len(
            [r for r in pending_outcome_links()
...
def _selftest() -> None:
    ...
    try:
        ...
        report = {
            "target": "owner/repo#5", "agent": "cursor", "lane": "opener",
            "task_type": "implement", "pid": 424242, "state": "stalled",
            "recommended_action": "inspect", "log": str(tmp / "a.log"),
            "worktree": str(tmp), "expected_paths": ["src"],
        }
        proposal = {
            "action": "redirect", "reason": "stale auth token", "confidence": "high",
        ...
        ran: list = []
        dry = apply_one(report=report, acceptance_criteria="All endpoints return 200.",
                        backend="codex", corpus_path=corpus, env={},
                        role_runner=role_runner, pid_checker=lambda pid: False,
        ...
        live = apply_one(report=report, acceptance_criteria="AC", backend="codex",
                         corpus_path=corpus, env={BOOTSTRAP_FLAG: "1"},
                         role_runner=role_runner, pid_checker=lambda pid: True,
                         apply_runner=must_not_run)
        ...
        unstamped = authorize(
            plan_obj={"action": "redirect", "target": "o/r#1", "prompt_text": "x",
                      "prompt_file": "f", "steps": [{"id": "delegate-retry",
                                                     "commands": [["python3", "d.py"]]}]},
            role_run_id="role:redirect:codex:1", decision_source="redirect_agent", errors=[],
            pid_alive=False, claim_holder=None, prior_agent="cursor",
            gate={"bootstrap_needed": True, "disagreements_needed": 3},
            applied_targets=set(), applies_today=0, flag_on=True,
        )
        ...
        stolen = authorize(
            plan_obj={"action": "redirect", "target": "o/r#1", "prompt_text": "x",
                      "prompt_file": "f", "accepted_role_run_id": "role:redirect:codex:1",
                      "steps": [{"id": "delegate-retry", "commands": [["python3", "d.py"]]}]},
            role_run_id="role:redirect:codex:1", decision_source="redirect_agent", errors=[],
            pid_alive=False, claim_holder={"agent": "gemini"}, prior_agent="cursor",
            gate={"bootstrap_needed": True, "disagreements_needed": 3},
            applied_targets=set(), applies_today=0, flag_on=True,
        )
        ...
        hot = apply_one(report={**report, "target": "owner/repo#6"},
                        acceptance_criteria="AC", backend="codex", corpus_path=corpus,
                        env={BOOTSTRAP_FLAG: "1"}, role_runner=role_runner,
                        pid_checker=lambda pid: False, apply_runner=fake_runner)
        ...
        repeat = apply_one(report={**report, "target": "owner/repo#6"},
                           acceptance_criteria="AC", backend="codex", corpus_path=corpus,
                           env={BOOTSTRAP_FLAG: "1"}, role_runner=role_runner,
                           pid_checker=lambda pid: False, apply_runner=fake_runner)
        assert repeat["authorization"]["allowed"] is False, repeat["authorization"]
        assert any("already been applied" in b or "daily bound" in b
                   for b in repeat["authorization"]["blocks"]), repeat["authorization"]

        # ---- the linker: recorded edge + terminal outcome -> corpus link ---------------
        role_run_id = hot["role_run_id"]
        feedback.record_run("work:applied-redirect", "owner/repo#6", "implement", "codex",
                            influenced_by_role_run_ids=[role_run_id])
        assert link_applied_outcomes(corpus_path=corpus)["linked"] == 0, "no outcome yet"
        feedback.record_outcome("work:applied-redirect", adjudicated_verdict="PASS",
        ...
        feedback.record_role_run("role:redirect:codex:rejected", "redirect", "owner/repo#9",
                                 "codex")
        feedback.record_run("work:rejected-redirect", "owner/repo#9", "implement", "codex")
        feedback.record_influence_edge(
            target_run_id="work:rejected-redirect", influence_type="role",
            influence_id="role:redirect:codex:rejected",
            source_run_id="role:redirect:codex:rejected", accepted=False,
        )
        feedback.record_outcome("work:rejected-redirect", adjudicated_verdict="PASS",
                                merged=True, durability="durable")
        assert link_applied_outcomes(corpus_path=corpus)["linked"] == 0, \
        ...
        clean = screen_report({"target": "owner/repo#77", "agent": "cursor", "pid": 424243},
                              gate=open_gate, applied_targets=set(), applies_today=0,
                              pid_checker=lambda pid: False)
        assert clean["passes_screen"] is True, clean
        alive = screen_report({"target": "owner/repo#77", "agent": "cursor", "pid": 1},
                             gate=open_gate, applied_targets=set(), applies_today=0,
                             pid_checker=lambda pid: True)
        assert alive["passes_screen"] is False
        assert any("still alive" in b for b in alive["blocks"]), alive
        closed = screen_report({"target": "owner/repo#77", "agent": "cursor", "pid": 424243},
                               gate={"bootstrap_needed": False}, applied_targets=set(),
                               applies_today=0, pid_checker=lambda pid: False)
        assert closed["passes_screen"] is False
        assert any("deficits are closed" in b for b in closed["blocks"]), closed
        bounded = screen_report({"target": "owner/repo#77", "agent": "cursor", "pid": 424243},
                                gate=open_gate, applied_targets=set(),
                                applies_today=MAX_APPLIES_PER_DAY, pid_checker=lambda pid: False)
        ...
        guarded = apply_candidates(
            report_dir=tmp, corpus_path=corpus, dry_run=True, env={},
        ...
        assert src in {"ambient", "tick", "unset", "unresolved (tick env unavailable; ambient unset)"}, src
        ...
        print("redirect_apply.py selftest: OK (flag-off dry run, live-pid refusal, unstamped "
...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument("--spend-offloads", action="store_true",
    ...
    ap.add_argument("--report-dir",
                    help="directory of keepalive-supervisor redirect reports (default: its own)")
    ...
    if args.screen:
        ...
        if args.json:
            ...
            print(f"screen: {out['passing_screen']} of {out['reports_seen']} live candidates could "
    ...
    out = apply_candidates(report_dir=Path(args.report_dir) if args.report_dir else None,
                           limit=args.limit, corpus_path=corpus,
                           acceptance_criteria=args.acceptance_criteria,
                           backend=args.backend or None, dry_run=args.dry_run,
                           spend_offloads=args.spend_offloads)
    if args.json:
        print(json.dumps(out, indent=2, default=str))
    else:
        print(f"reports={out['reports_seen']} considered={out['considered']} "
              f"authorized={out['authorized']} applied={out['applied']} "
              f"(flag {BOOTSTRAP_FLAG}={'1' if flag_enabled() else '0'})")
        for row in out["results"]:
            verdict = ("APPLIED" if row.get("applied") else
                       "authorized (flag off)" if row.get("authorized") else "refused")
