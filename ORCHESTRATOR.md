...
- **Assess capacity** — `python3 capacity.py` → per-agent `{ok|warn|shed}`. Who has headroom right
  now? (codex/claude default OK + 429-shed; cursor=free/unlimited; vibe=subscription; aider=paygo (LOCAL_POLICY.md).)
- **Observe fleet health** — `python3 observability_dashboard.py [--json] [--write-markdown path]`
...
- **Discover work** — `python3 backlog.py --dry-run` → actionable items `{target, task_type, lane}`
  (ready issues + in-flight agent PRs across the fleet; scope-blocked excluded). `--live` refreshes.
- **Consult the rule-based prior (OPTIONAL)** — `python3 router.py --dry-run` → what a deterministic
  planner *would* do. A second opinion to weigh, not an instruction. Ignore it when your read differs.
- **Delegate one task** — `python3 dispatcher.py delegate --agent <cursor|vibe|codex|claude|aider>
...
- **Offload one synchronous read/proposal** — `python3 dispatcher.py offload --agent <cursor|vibe|gemini|codex>
...
- **Automatic local watch sweep (NEW)** — `python3 redirect_sweep.py [--write path] [--json]` scans
  ...
  enables this when `ORCH_REDIRECT_SWEEP_RECORD_CORPUS=1`. Use `python3 redirect_sweep.py --doctor
...
- **Plan redirect/decompose (NEW)** — `python3 redirect_plan.py --report-json watch-report.json
...
- **Run an agent-role in shadow (NEW)** — `python3 roles.py route --role redirect` shows the
  router-chosen backend for a role; `python3 roles.py redirect --report-json <watch-report>.json
  ...
  `python3 roles.py link-outcome --role-run-id RID --influenced-run-id DOWNSTREAM_RID` so the learner can
  update `role:<name>` backend fit. Roles are typed contracts with swappable backends; see
  [`ARCHITECTURE.md`](ARCHITECTURE.md). Selftest: `python3 roles.py --selftest`.
- **Measure RedirectAgent before promoting it (NEW)** — use
  `python3 redirect_shadow.py record --report-json <watch-report>.json --ac "<acceptance criteria>" --dispatch`
  on real stalls to append a local, shadow-only proposal event; use
  `python3 redirect_shadow.py summarize` / `python3 roles.py summarize-proposals` to review proposal
  ...
  Use `python3 redirect_shadow.py historical-candidates` to find old keepalive-shadow PRs worth replaying
...
- **Plan post-escalation keepalive supervision (NEW)** — `python3 keepalive_supervisor.py --list-live
  ...
  `python3 keepalive_supervisor.py --stage2-plan [--stage2-backend cursor] [--historical-backend cursor]
...
- **Run PromptAgent in shadow (NEW)** — `python3 roles.py route --role prompt` shows the router-chosen
  backend for prompt authoring; `python3 roles.py prompt --target owner/repo#N --goal "..." --task-type
...
- **Run DecomposerAgent in shadow (NEW)** — `python3 roles.py route --role decomposer` shows the
  router-chosen backend for decomposition; `python3 roles.py decompose --goal "..." --repo owner/repo
...
- **Run TriageAgent in shadow (NEW)** — `python3 roles.py route --role triage` shows the router-chosen
  backend for backlog triage; `python3 roles.py triage --backlog-json ~/.codex/handoff/backlog.json
...
- **Run AdjudicatorAgent in shadow (NEW)** — `python3 roles.py route --role adjudicator` shows the
  router-chosen backend for disputed-blocker adjudication; `python3 roles.py adjudicate --case-json
...
- **Redirect** — `kill <pid>` → `claims.py release <target>` → re-`delegate` with a different agent
  or a sharper prompt.
- **Verify frontend (NEW)** — `python3 frontend_verify.py --url <served-url> --assert "text:<s>"
  ...
  Use `python3 frontend_verify.py --doctor [--require-browser-endpoint] [--json]` before cron/sandboxed
...
- **Build or validate an epic decomposition plan (NEW)** — `python3 epic_lane.py --goal "<goal>"
  [--repo owner/repo] [--target owner/repo#N] [--context-file brief.md] [--subtask-count N]` emits the
  strict planner prompt for a large/vague goal; `python3 epic_lane.py --validate plan.json --json
  ...
  says to reserve it.
- **Build or validate a codemod/refactor campaign (NEW)** — `python3 codemod_lane.py --goal "<goal>"
  [--repo owner/repo] [--target owner/repo#N] [--context-file brief.md]` emits a strict campaign-authoring
  prompt; `python3 codemod_lane.py --validate campaign.json` validates agent-produced campaign JSON;
  `python3 codemod_lane.py --plan campaign.json --json [--emit-delegation-prompt]` produces a dry-run plan
...
- **Build or validate a cross-repo coordinated-change plan (NEW)** — `python3 cross_repo_lane.py --goal
  ...
  `python3 cross_repo_lane.py --validate coordination.json` validates source/consumer rollout JSON;
  `python3 cross_repo_lane.py --plan coordination.json --json [--emit-dispatch-prompts]` produces a dry-run
...
- **Ingest consumer-sync evidence without consumer writes (NEW)** —
  `python3 consumer_sync_artifact_ingest.py preview` validates the latest successful
...
- **Build or validate a runtime AC verification plan (NEW)** — `python3 runtime_ac.py --goal "<goal>"
  [--repo owner/repo] [--target owner/repo#N] [--context-file brief.md]` emits a strict
  acceptance-criteria evidence-authoring prompt; `python3 runtime_ac.py --validate spec.json` validates
  AC-bound verification JSON; `python3 runtime_ac.py --plan spec.json --json [--emit-commands]` produces
  a dry-run plan with review-before-run commands for `frontend_verify.py`, `local_verify.py`, command
  checks, and manual evidence. `python3 runtime_ac.py --run spec.json --confirm-run` executes selected
  ...
  `python3 runtime_ac.py --results spec.json --result-json results.json` to gate externally collected
  ...
  `python3 runtime_ac_gate.py --scan-backlog [--json]` to inspect the current backlog for
  ...
  `python3 runtime_ac_gate.py --materialize-range-spec spec.json --target owner/repo#N --json`
...
- **Roll out range lanes from backlog (NEW)** — `python3 range_lane_rollout.py [--json]
...
- **Guard a terminal merge with runtime AC (NEW)** — `python3 merge_guard.py owner/repo#N` dry-runs the
  merge command and reports whether runtime AC is required. `python3 merge_guard.py owner/repo#N
  ...
  terminal merges enforce the same policy. Use `python3 runtime_ac_gate.py --exercise [--json]` for a
  ...
  `python3 runtime_ac_flow_monitor.py --json` for current truth: firing comes from structured
...
- **Adjudicate runtime AC with a panel (NEW)** — `python3 runtime_ac_panel.py --prompt spec.json --gate
  ...
  `python3 runtime_ac_panel.py --adjudicate spec.json
  ...
  patching feedback. To run the reviewer panel directly, use `python3 runtime_ac_panel.py --dispatch
...
- **Run a local deliberate-break gate (NEW)** — `python3 local_verify.py --worktree <wt>
...
- **Ingest LangSmith trace artifacts (NEW)** — `python3 langsmith_pull.py --ndjson <langsmith-fleet.ndjson>
...
- **Pull direct LangSmith API telemetry (NEW)** — `python3 langsmith_direct.py --dry-run --json`
  previews, and `python3 langsmith_direct.py --ingest [--json]` writes, `workflows-agents` telemetry from
...
- **Fetch LangSmith fleet artifacts (NEW)** — `python3 langsmith_fetch.py --ingest [--json]` reads the
...
- **Reconcile local execution ledger (NEW)** — `python3 ledger_reconcile.py reconcile [--dry-run] [--json]`
...
- **Attribute ccusage sessions to runs (NEW)** — `python3 ccusage_reconcile.py reconcile --dry-run --json`
  previews, and `python3 ccusage_reconcile.py reconcile [--json]` writes, per-run Codex/Claude usage rows
...
- **Ingest keepalive process outcomes (NEW)** — `python3 keepalive_outcomes.py --include-non-agent`
...
- **Inject repo playbooks (NEW)** — `python3 repo_knowledge.py <owner/repo[#N]> [task_type] [lane]` previews
  ...
  Use `python3 repo_knowledge.py --search owner/repo[#N] [--query TEXT] [--task-type T] [--lane L]`
  ...
  Use `python3 repo_knowledge.py --suggest-from-snapshot data/feedback-snapshot.json` to surface candidate
...
- **Periodic dataset report (NEW)** — `python3 periodic_report.py [--json] [--window-days N]
  ...
  `python3 dry_seam_audit.py [--json]` is the
...
- **Record task-end feature reflection (NEW)** — `python3 features.py record --name <feature>
  --where <task-or-run> --problem "<future problem solved>" [--module module.py] [--maturity ad-hoc|reused|hardened]`
  logs reusable structures into `experiments/features.json`. Use `python3 features.py summary --json` for
  maturity counts and top reused structures, `python3 features.py candidates` for rule-of-three promotion
  candidates, and `python3 features.py harden --name <feature> --module <module>` when a pattern becomes a
  selftested module. `periodic_report.py` reads this registry without creating or mutating it.
- **Check judge reliability (NEW)** — `python3 judge_reliability.py [--json]` reports data-gated evaluator
...
- **Promote a verified synthesis without publishing it (NEW)** — the normal `exp_abcd.py followup`
  ...
  use `python3 synthesis_promotion.py link-delivery <experiment-dir> --run-id RUN --ref owner/repo#N`
...
- **Check human calibration readiness (NEW)** — `python3 human_calibration.py [--json]` parses structured
...
- **Cluster evidence gaps into schema candidates (NEW)** — `python3 evidence_schema.py [--json]` groups
  ...
  explicit and guarded: `python3 evidence_schema.py --apply NAME --confirm-type NAME`; approval records the
...
- **Ingest delegated outcomes (NEW)** — `python3 outcomes.py --mode remote|local|both [--dry-run]`
...
`python3 exploration_evidence_plan.py` to inspect remaining ε-greedy / Thompson-hybrid evidence deficits,
then `python3 exploration_collection.py` to dry-run a bounded collection window. Active dispatch is never
implicit: it requires `ORCH_EXPLORATION_EVIDENCE=1 python3 exploration_collection.py --apply
...
**Route-coverage backfill (built 2026-06-22):** use `python3 exploration_backfill.py` to inspect missing
...
guarded: `ORCH_EXPLORATION_BACKFILL=1 python3 exploration_backfill.py --apply --confirm-backfill
...
**Strategy-value experiments (H4/H5, built 2026-06-23):** use `python3 strategy_experiment.py --hypothesis
...
`ORCH_STRATEGY_EXPERIMENT=1 python3 strategy_experiment.py ... --prepare --confirm-strategy`. The normal
