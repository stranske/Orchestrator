...
def _adversarial_context(item: dict, reason: str) -> str:
    ...
    return f"High-stakes closer PR {item.get('target')}: {title}. Reason: {reason}. Labels: {labels}."
...
def _adversarial_review_status(
    ...
    *,
    ...
    if not adversarial.review_enabled(env):
        return {**base, "status": "required_but_not_run",
    ...
    try:
        ...
        return {**base, "status": "executed", "reviewers": reviewers, "worktree": str(worktree),
...
def research_tick(items: list[dict], cap: dict, *, learned: dict | None = None, dry_run: bool = True,
                  ...
                  per_subject_cap: int = research_subjects.DEFAULT_PER_SUBJECT_CAP) -> dict:
    ...
    return {**{k: v for k, v in public.items() if k != "planned"},
            "status": "planned" if plans else "no_plan", "planned": plans,
            "active": bool(launched), "launched": launched}


def remote_tick(items: list, cap: dict, *, learned: dict | None = None, dry_run: bool = True,
                do_ingest: bool = True, max_delegations: int | None = None,
                env: dict | None = None, runtime_ac_gate_fn=None, research_tick_fn=None) -> dict:
    ...
    if not dry_run:
        claims.reap_stale()
    cap_n = max_delegations if max_delegations is not None else int(os.environ.get("ORCH_MAX_REMOTE_PER_TICK", "3"))
    chosen, no_capacity, deferred, blocked, adversarial_reviews, runtime_ac_gates = [], [], [], [], [], []
    role_shadows: list[dict] = []
    triage_shadow = roles.activate_tick_triage(
        items, cap, env=env, dry_run=dry_run
    )
    role_shadows.append({
        "role": "triage",
    ...
    for item in items:
        if len(chosen) >= cap_n:                      # per-tick cap: defer the rest (cost guard)
            deferred.append(item.get("target")); continue
        tt = item.get("task_type", "implement")
        held = claims.holder(str(item.get("target")))
        if held:
            blocked.append({"target": item.get("target"), "task_type": tt,
        ...
        if gate_status:
            ...
            if gate_status.get("blocks"):
                ...
                role_shadows.append({
                    "role": "adjudicator", "target": item.get("target"),
        ...
        if not pick:
            no_capacity.append({"target": item.get("target"), "task_type": tt,
        ...
        triage_agrees = recommendation.get("action") in {
            "work_now", "monitor"
        }
        ...
        if not dry_run:
            ...
            for role_run_id in rejected_ids:
                feedback.record_influence_edge(
                    target_run_id=downstream_run_id, influence_type="role",
                    influence_id=role_run_id, source_run_id=role_run_id,
                    accepted=False, metadata={"status": "shadow_only", "disagreement": True},
                )
        chosen.append({"target": item.get("target"), "task_type": tt, "agent": pick["agent"],
                       "applied": res.get("applied"), "skip": res.get("skip"), "dry_run": dry_run})
    ...
    ingest = outcomes.ingest_outcomes(dry_run=dry_run) if do_ingest else {"note": "skipped (do_ingest=False)"}
    return {"chosen": chosen, "no_capacity": no_capacity, "deferred": deferred, "blocked": blocked,
            "ingest": ingest, "dry_run": dry_run, "cap": cap_n,
            "adversarial_reviews": adversarial_reviews, "runtime_ac_gates": runtime_ac_gates,
...
def _selftest():
    ...
    try:
        ...
        items = [{"target": "stranske/Workflows#101", "task_type": "implement"},
                 {"target": "stranske/Counter_Risk#5", "task_type": "mechanical"}]
        out = remote_tick(items, all_keep, dry_run=True, do_ingest=False, research_tick_fn=no_research)
        assert len(out["chosen"]) == 2 and out["dry_run"] is True, out
        assert all(c["agent"] != "claude" for c in out["chosen"]), out["chosen"]   # reserve-aware: routine avoids claude
        assert out["chosen"][0]["agent"] == "codex", out["chosen"]                 # implement -> codex (non-reserve lead)
        assert out["chosen"][1]["agent"] == "cursor", out["chosen"]                # mechanical -> cursor
        learned = {"implement": {"gemini": {"rank": 0, "n_obs": 2}}}
        learned_out = remote_tick([{"target": "o/r#learned", "task_type": "implement"}], all_keep,
                                  learned=learned, dry_run=True, do_ingest=False,
                                  research_tick_fn=no_research)
        assert learned_out["chosen"][0]["agent"] == "gemini", learned_out          # task-specific learned slice is used
        out2 = remote_tick([{"target": "o/r#1", "task_type": "implement"}],
                           cap({"vibe": "ok", "cursor": "shed", "codex": "shed", "claude": "shed", "gemini": "shed"}),
                           dry_run=True, do_ingest=False, research_tick_fn=no_research)
        ...
        out3 = remote_tick(items, all_keep, dry_run=True, do_ingest=False, max_delegations=1,
                           research_tick_fn=no_research)
        ...
        assert claims.holder(production_range["target"]) is None, (
            "production-reserved dry-run created a research claim"
        )
        high = {"target": "stranske/Workflows#202", "task_type": "implement", "lane": "closer",
                "labels": ["risk:high"], "title": "security-sensitive workflow change"}
        out4 = remote_tick([high], all_keep, dry_run=True, do_ingest=False, research_tick_fn=no_research)
        ...
        ran = _adversarial_review_status(
            ...
            review_fn=lambda worktree, reviewers, context: {"verdict": "PASS", "n_vetoes": 0,
                                                            "reviewers": reviewers, "context": context},
        )
        assert ran["status"] == "executed" and ran["reviewers"] == ["vibe", "gemini"], ran
        assert ran["result"]["verdict"] == "PASS", ran
        assert _adversarial_review_status({"target": "o/r#1", "lane": "closer", "labels": []},
        ...
        assert fired[0]["task_type"] == "testgen", (
            f"expected testgen, got {fired[0]['task_type']}"
        )
        ...
        gated = research_tick(
            ...
            env={},
            conn=subject_conn,
            prepare_fn=lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("must not launch")),
            issue_body_fn=lambda target: "Concrete frozen spec",
            hyps=research_scheduler.SEED_HYPOTHESES,
            rng=__import__("random").Random(0),
            unevaluated_cap=1000,
        ...
        assert no_spare_research["status"] == "no_spare" and not no_spare_research["planned"], no_spare_research

        with tempfile.TemporaryDirectory(prefix="runtime-ac-gate-") as tmp:
            runtime_item = {"target": "stranske/Workflows#303", "task_type": "implement", "lane": "closer",
                            "labels": ["runtime-ac"], "title": "Runtime-sensitive merge"}
