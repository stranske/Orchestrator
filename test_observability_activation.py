...
def _minimal_report() -> dict:
    return {
        ...
        "production_flow": {
            "status": "flowing",
            "production_runs": 100,
            "production_outcomes": 99,
            "outcome_coverage": 0.99,
            "recent_production_runs": 100,
            "recent_production_outcomes": 99,
        },
        "dry_seams": {
            "overall": "warn",
...
def test_production_denominator_is_headline_not_expected_shadow_rows(
    ...
    monkeypatch.setattr(
        dashboard, "_cadence_health", lambda: cadence_registry.inspect_cadence(tmp_path, now=1)
    )
    built = dashboard.build_dashboard(report, capacity_snapshot={})
    assert (
        built["scorecard"]["production_outcome_coverage"] == 0.99
...
def test_exact_activation_reasons_render_distinctly(
    ...
    assert (
        activation["runtime_ac"]["exact_reason"]
        == "closer lacks runtime-AC label or verification spec"
    )
