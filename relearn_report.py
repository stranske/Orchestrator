...
def _role_priors_from_registry(task_priors: dict) -> dict:
    ...
    for role_name, role in getattr(roles, "ROLE_REGISTRY", {}).items():
        ...
        priors = {
            agent: prior
            for agent, prior in base.items()
            if agent in role.eligible_backends
        }
        if priors:
...
def _learner():
    if _quality_learner_writes_current_weight_table():
        return "feedback.relearn_quality", feedback.relearn_quality, (
            "preferred continuous-reward learner; it writes route_weights read by current_weights"
        )
    return "feedback.relearn", feedback.relearn, (
...
def _version_exists(version: int | None) -> bool:
    if version is None or version < 1:
        return False
    with feedback._conn() as c:
        row = c.execute("SELECT 1 FROM route_weights WHERE version=? LIMIT 1", (version,)).fetchone()
...
def _task_report(task_type: str, priors: dict, new_version: int, previous_version: int | None) -> dict:
    ...
    previous_order = [row["agent"] for row in previous_rows] if previous_version is not None else None
...
def format_human(report: dict) -> str:
    ...
    if report["dry_run"]:
        ...
        for task in report["tasks"]:
            ...
            for row in task["rows"]:
                lines.append(f"  {row['prior_rank']}. {row['agent']} prior={_fmt_num(row['prior'])}")
        return "\n".join(lines)
    ...
    lines.append(report["learner_note"])
    lines.append("Rank deltas are learned_rank minus comparison_rank; negative means the agent rose.")
    ...
    for task in report["tasks"]:
        ...
        if task["previous_order"] is not None:
            lines.append(f"  previous:{' > '.join(task['previous_order']) if task['previous_order'] else ' none'}")
        if task["note"]:
...
def _selftest():
    ...
    try:
        ...
        route_table = {"implement": {"role": "code", "agents": [
        ...
        for i in range(12):
            ...
            feedback.record_outcome(rid, adjudicated_verdict="PASS", merged=True, durability="reverted")
        ...
        bt_priors = {"implement": {"cursor": 0.70, "codex": 0.65},
                     "review": {"cursor": 0.70, "codex": 0.65}}

        def fake_bt(task_type=None, **_kw):
            if task_type == "implement":
                return {"ready": True, "comparisons": 12,
        ...
        outcome_route_table = {"outcome_only": {"role": "code", "agents": [
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Run feedback relearning and report route weights.")
    ...
    parser.add_argument("--dry-run", action="store_true", help="compute priors without writing weights")
