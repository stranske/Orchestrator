...
def _proposals() -> dict[str, dict]:
    ...
    return {
        ...
        "prompt": {
            ...
            "risk_flags": ["Preserve compatibility"],
            "confidence": "high",
        },
        ...
        "adjudicator": {
            ...
            "evidence_assessment": [
                {
                    "claim": "runtime evidence is missing",
                    "status": "contradicted",
                    "evidence_ref": "runtime-ac:1",
                    "reason": "The persisted check passed.",
                }
            ],
...
def _invoke(role_name: str) -> dict:
    ...
    if role_name == "decomposer":
        return roles.run_decomposer_agent(
            goal="Deliver the bounded epic", target="owner/repo#1", **common
        )
    if role_name == "triage":
...
def test_accepted_role_outcome_auto_links(
    ...
    role_run_id = result["role_run_id"]
    dispatch = {
        "run_id": "dispatch:accepted-proposal",
        "agent": "cursor",
        "mode": "composer",
        "target": "owner/repo#1",
        "lane": "opener",
...
def test_rejected_role_records_disagreement_without_copied_success(
    ...
    feedback.record_influence_edge(
        target_run_id="work:rejected-role",
        influence_type="role",
        influence_id=result["role_run_id"],
        source_run_id=result["role_run_id"],
        accepted=False,
...
def test_selector_distinguishes_no_match_from_gate_capacity_and_cap(
    ...
    assert (
        gate["selector_status"] == "matched_not_invoked"
        and gate["reason"] == "shadow_gate_disabled"
    )
    assert (
        capacity["selector_status"] == "matched_not_invoked"
        and capacity["reason"] == "no_role_capacity"
    )
    assert invoked["selector_status"] == "invoked"
    assert (
        capped["selector_status"] == "matched_not_invoked"
        and capped["reason"] == "per_cycle_invocation_cap"
    )
